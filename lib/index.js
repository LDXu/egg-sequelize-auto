 ...
 * @Last Modified by: xubill
 * @Last Modified time: 2021-04-20 17:08:09
 */
  ...
  run(callback) {
    ...
    function generateText(err) {
      ...
      async.each(_.keys(self.tables), function(table, _callback) {
        ...
        //约定数据库表 小驼峰。此处只把首字母修改成大写        
        let modelName = tableName.replace(tableName.slice(0, 1), tableName.slice(0, 1).toUpperCase())
        text[table] += spaces + "const " + modelName + " = app.model.define('" + tableName + "', {\n";
          ...
          _.each(fieldAttr, function(attr, x) {
            ...
            if (attr === "foreignKey") {
              ...
              if (_.isString(defaultVal)) {
                ...
                if (field_type.indexOf('date') === 0 || field_type.indexOf('timestamp') === 0) {
                  if (_.endsWith(defaultVal, '()')) {
                    ...
                    val_text = "app.Sequelize.literal('" + defaultVal + "')"
                  } else {
              ...
              if (defaultVal === null || defaultVal === undefined) {
                ...
                val_text = _.isString(val_text) && !val_text.match(/equelize\.[^(]+\(.*\)$/) ? SqlString.escape(_.trim(val_text, '"'), null, self.options.dialect) : val_text;
        ...
        text[table] += ");\n\n";
        // associate 
        text[table] += spaces + modelName + ".associate = function() {\n\n" + spaces + "}\n\n";
        text[table] += spaces + "return " + modelName + ";\n";
        //resume normal output
        text[table] += "};\n";
        _callback(null);
      }, function() {
